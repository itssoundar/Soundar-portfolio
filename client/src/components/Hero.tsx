...
export function Hero() {
  ...
  return (
    ...
    >
      ...
      >
        {/* Ticker Container */}
        <div className="flex w-max animate-ticker hover:animation-play-state-paused min-h-[80px] md:min-h-[100px] items-center">
          {/* First set of logos */}
          <div className="flex justify-around items-center py-8 md:py-6">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((num) => (
              <div key={`logo-1-${num}`} className="px-3 md:px-[40px]">
                <img 
                  src={`/works/worked_0${num}.png`} 
                  alt={`Company ${num}`} 
                  className="h-20 md:h-20 w-auto max-w-[140px] md:max-w-[200px] object-contain flex-shrink-0 grayscale opacity-90 hover:opacity-100 hover:grayscale-0 transition-all duration-300 mix-blend-multiply"
                />
              </div>
            ))}
          </div>
