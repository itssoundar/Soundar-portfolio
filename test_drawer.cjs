...
// I'm changing the trigger element. Sometimes the `absolute inset-0` trick is confusing browsers or the user wants the exact behavior of a bottom sheet opening correctly.
// Let's switch back to a standard <button> wrapping the entire card, which is natively accessible and handled well by Vaul.
