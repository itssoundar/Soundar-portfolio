...
content = content.replace(
  /const renderCardContent = \(project: typeof projects\[0\], isFirst: boolean = false\) => \([\s\S]*?<\/button>\n  \);/g,
  `const renderCardContent = (project: typeof projects[0], isFirst: boolean = false) => (
    <button 
      ...
      className="block w-full bg-white rounded-[28px] p-[16px] md:p-[32px] shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-[#eaeaea] group transition-all duration-300 md:hover:shadow-[0_8px_32px_rgba(0,0,0,0.08)] relative z-[60] text-left appearance-none" style={{ WebkitAppearance: "none", touchAction: "manipulation", WebkitTapHighlightColor: "transparent" }}>
