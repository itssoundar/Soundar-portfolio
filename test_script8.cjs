...
// Use a regular generic div that simply has an onClick handler, and remove all complexity.
// Let's strip out all the fancy cursor/touch styling on the root element.

content = content.replace(
  /const renderCardContent = \(project: typeof projects\[0\], isFirst: boolean = false\) => \([\s\S]*?<\/a>\n  \);/g,
